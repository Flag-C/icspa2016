...
uint32_t lnaddr_read(lnaddr_t addr, size_t len) {
	...
	if ((cpu.cr0.protect_enable == 1) && (cpu.cr0.paging == 1))
	{
		if ((addr & 0xfffff000) != ((addr + len - 1) & 0xfffff000)) {
			...
			Log("read cross page,lnaddr=%x,hwaddr1=%x,hwaddr2=%x", addr, page_translate(prev_page_addr), page_translate(sec_page_addr));
			return (hwaddr_read(page_translate(sec_page_addr), sec_len) << prev_len)
			       + hwaddr_read(page_translate(prev_page_addr), prev_len);
		}
		else {
